...
pub struct ReuploadInvoiceCommand {
  pub user_id: Uuid,
  pub invoice_id: Uuid,
}

#[derive(Debug, Serialize)]
pub struct ReuploadInvoiceResponse {
  pub invoice_id: Uuid,
  pub pdf_path: Option<String>,
}

pub struct ReuploadInvoiceUseCase {
...
impl ReuploadInvoiceUseCase {
  ...
  pub async fn execute(
    &self,
    command: ReuploadInvoiceCommand,
  ) -> Result<ReuploadInvoiceResponse, InvoiceError> {
    // Fetch invoice and verify it exists
    let invoice = self
      .invoice_service
      .get_invoice(command.user_id, command.invoice_id)
      .await?;

    // Guard: only non-draft, non-cancelled invoices can have PDFs re-uploaded
    if invoice.status == InvoiceStatus::Draft || invoice.status == InvoiceStatus::Cancelled {
      return Err(InvoiceError::CannotEditInvoice(format!(
        "Cannot re-upload PDF for invoice with status '{}'",
    ...
    // Update stored PDF path and Drive file ID
    let updated_invoice = self
      .invoice_service
      .set_invoice_pdf_path(command.invoice_id, pdf_path.clone(), Some(file_id))
      .await?;

    Ok(ReuploadInvoiceResponse {
      invoice_id: updated_invoice.id,
      pdf_path: updated_invoice.pdf_path,
    })
  }
}
